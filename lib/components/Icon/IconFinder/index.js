...
export class IconFinder extends React.Component {
    constructor(props) {
        super(props);
        this.name = prefix + "icon-finder";
        this.state = {
            value: "",
        };
    }
    ...
    renderIcons() {
        ...
        return found.map(obj => {
            ...
            return (React.createElement(Image, { key: obj, className: `${this.name}__icon`, title: obj, tabIndex: 1 },
                React.createElement(Icon, { name: `${obj}`, size: 30, text: `import ${obj} from "@reglendo/mergado-ui-icons/lib/icons/${obj}"`, viewBox: viewBox, color: "#555", style: { display: "inline-block", margin: "5px" } })));
        });
    }
    render() {
        const className = `${this.name}`;
        return (React.createElement("div", { className: `${className}` },
            React.createElement(TextInput, { labels: { main: "",
