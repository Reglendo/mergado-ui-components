...
class CheckboxContainer extends React.Component {
    ...
    renderFilter() {
        return (React.createElement(TextInput, { className: `${this.name}__filter_input`, type: "search", onClear: () => this.setState({ filter: "" }), style: { marginBottom: "5px" }, input: { onKeyUp: this.handleFilter }, labels: { placeholder: this.props.labels.placeholder, main: "" } }));
    }
    handleFilter(evt) {
    ...
    render() {
        ...
        return (React.createElement(StyledField, Object.assign({}, props, { labels: labels, className: `${this.name}__queries` }),
            ...
            React.createElement(QueryList, { className: `${this.name}__list ${isInvalid ? `${form}__group--invalid` : ""}`, name: this.props.name, height: height, activeFirst: props.activeFirst, options: options, value: this.props.input.value ? this.props.input.value : [], input: this.props.input, singleChoice: this.props.singleChoice, showInput: this.props.showInput, labels: labels, meta: meta })));
    }
}
