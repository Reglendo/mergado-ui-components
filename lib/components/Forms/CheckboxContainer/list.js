...
const renderOptions = (name, options, value, input, singleChoice, showInput, labels) => {
    ...
    return options
        ...
        return React.createElement(QueryItem, { name: name, option: option, index: index, onClick: handler, checked: value.indexOf(option.id) > -1, singleChoice: singleChoice, value: value, showInput: showInput, labels: labels, key: option.id + option.name });
    });
};
